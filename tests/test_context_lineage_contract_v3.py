...
class TestContextLineageContract(unittest.TestCase):
    def test_context_resolves_active_goal_and_task_objects(self):
        ...
        task = Task(title="create X content", goal_id=goal.id, id="task-1")
        state = SystemState(active_goal_id=goal.id, active_task=task.id)
    ...
    def test_context_rejects_task_from_different_goal(self):
        ...
        task = Task(title="create X content", goal_id="goal-2", id="task-1")
        state = SystemState(active_goal_id=goal.id, active_task=task.id)
